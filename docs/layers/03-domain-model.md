...
TDD §5.1: "`type` answers 'what kind of memory' (`fact | goal | preference | interest`); `category` answers 'which life area' and uses the same taxonomy as query classification (§4.5), so retrieval is a direct equality match." So `career.goal` is `type = goal`, `category = career`; `language.preferred` is `type = preference`, `category = language`; `profile.name` is `type = fact`, `category = profile`; `interests.cricket` is `type = interest`, `category = interests`.
